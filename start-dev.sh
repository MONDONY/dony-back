#!/usr/bin/env bash
# Démarrage backend yadony en dev : attend Docker -> stack docker (pg/minio) -> DB healthy -> Spring Boot
set -u
cd "$(dirname "$0")" || exit 1
...
echo "[start-dev] Attente DB yadony_db healthy..."
for i in $(seq 1 60); do
  status=$(docker inspect --format '{{.State.Health.Status}}' yadony_db 2>/dev/null)
