...
Mise en place de l'observabilité complète de l'API yadony en production : exposition des métriques Spring Boot Actuator via Micrometer / Prometheus, instrumentation des 12 événements de domaine métier via un listener centralisé (`BusinessMetricsListener`), et collecte/expédition vers Grafana Cloud par l'agent Alloy déployé en sidecar Docker.

## Fichiers créés

- `src/main/java/com/yadony/api/common/metrics/BusinessMetricsListener.java` — listener centralisé écoutant 12 événements de domaine Spring et incrémentant les compteurs Micrometer correspondants
- `src/test/java/com/yadony/api/common/metrics/BusinessMetricsListenerTest.java` — 14 tests unitaires avec `SimpleMeterRegistry`
- `src/test/java/com/yadony/api/common/metrics/PrometheusEndpointIntegrationTest.java` — test d'intégration vérifiant l'accessibilité de l'endpoint `/actuator/prometheus`
...
- `src/main/java/com/yadony/api/config/SecurityConfig.java` — ajout de `/actuator/prometheus` en `permitAll` pour permettre le scrape par Alloy sans token Firebase
...
Pour `yadony.bids.created`, le tag `corridor` (ex. `Paris-Dakar`) est extrait de l'événement. Si la valeur est nulle ou vide, la méthode `safe()` substitue la valeur `"unknown"` pour éviter une exception Micrometer sur un tag null.

Pour `yadony.cancellations.confirmed`, le tag `reason` est extrait depuis l'enum de l'événement. S'il est null, `"unknown"` est utilisé par défaut.

### Mapping événement → compteur Micrometer

| Événement Spring | Compteur Micrometer | Nom Prometheus |
|---|---|---|
| `UserRegisteredEvent` | `yadony.users.registered` | `yadony_users_registered_total` |
| `AnnouncementCreatedEvent` | `yadony.announcements.created` | `yadony_announcements_created_total` |
| `BidCreatedEvent` | `yadony.bids.created{corridor=...}` | `yadony_bids_created_total` |
| `BidAcceptedEvent` | `yadony.bids.accepted` | `yadony_bids_accepted_total` |
| `BidRejectedEvent` | `yadony.bids.rejected` | `yadony_bids_rejected_total` |
| `PaymentEscrowReadyEvent` | `yadony.payments.escrow_ready` | `yadony_payments_escrow_ready_total` |
| `PaymentReleasedEvent` | `yadony.payments.released` | `yadony_payments_released_total` |
| `UserKycVerifiedEvent` | `yadony.kyc.verified` | `yadony_kyc_verified_total` |
| `DeliveryConfirmedEvent` | `yadony.deliveries.confirmed` | `yadony_deliveries_confirmed_total` |
| `DisputeOpenedEvent` | `yadony.disputes.opened` | `yadony_disputes_opened_total` |
| `CancellationConfirmedEvent` | `yadony.cancellations.confirmed{reason=...}` | `yadony_cancellations_confirmed_total` |
| `VoyageurNoShowEvent` | `yadony.travelers.no_show` | `yadony_travelers_no_show_total` |
...
Le service `alloy` dans les fichiers Docker Compose requiert 6 variables d'environnement : `GRAFANA_PROM_URL`, `GRAFANA_PROM_USER`, `GRAFANA_LOKI_URL`, `GRAFANA_LOKI_USER`, `GRAFANA_CLOUD_TOKEN`, `YADONY_ENV`. Sans elles, le conteneur démarrera mais n'enverra aucune donnée vers Grafana Cloud. Vérifier avec `docker logs yadony_alloy` après déploiement.
...
- [x] 12 compteurs métier yadony sont instrumentés — implémentés dans `BusinessMetricsListener` via `@EventListener` sur les événements de domaine existants
...
- Tests ajoutés :
  - `src/test/java/com/yadony/api/common/metrics/BusinessMetricsListenerTest.java` — 14 tests unitaires avec `SimpleMeterRegistry`, couvrant chaque handler de `BusinessMetricsListener` ainsi que les cas limites (corridor null, reason null)
  - `src/test/java/com/yadony/api/common/metrics/PrometheusEndpointIntegrationTest.java` — test d'intégration `@SpringBootTest` + `MockMvc` vérifiant que `GET /actuator/prometheus` retourne HTTP 200 et un contenu text/plain
