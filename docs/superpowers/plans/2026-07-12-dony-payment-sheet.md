# DonyPaymentSheet — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remplacer la PaymentSheet Stripe par une bottom sheet dony custom (wallets + PayPal + cartes enregistrées + nouvelle carte), pilote sur le checkout de bid.

**Architecture:** Backend : customer Stripe attaché au PaymentIntent + `setup_future_usage` optionnel + endpoint de liste des cartes. Flutter : `PaymentGateway` (abstraction testable du SDK Stripe) + `PaymentSheetBloc` + `DonyPaymentSheet` (3 vues sur `DonyBottomSheet`). Escrow/commission/webhooks inchangés.

**Spec:** `docs/superpowers/specs/2026-07-12-dony-payment-sheet-design.md` — les décisions produit et invariants y sont; la spec fait foi.

**Branches:** `feature/dony-payment-sheet` (dony-back, déjà créée) et `feature/dony-payment-sheet` (dony_app, à créer depuis main).
...
- **Bouton payer dans `stickyBottom`** de `DonyBottomSheet` (règle CLAUDE.md), jamais dans le child scrollable.
...
### Task 1 (dony-back): customer sur le PaymentIntent + savePaymentMethod
...
### Task 2 (dony-back): GET /payments/me/payment-methods
...
### Task 3 (dony_app): PaymentGateway + PaymentSheetBloc
...
### Task 4 (dony_app): DonyPaymentSheet UI (3 vues)

**Files:** Create `lib/features/payments/presentation/dony_payment_sheet.dart` (+ sous-widgets), tests widget.
...
### Task 5 (dony_app): bascule pilote checkout bid

**Files:** Modify `matching/presentation/screens/create_bid_screen.dart` (`_presentPaymentSheet` → `DonyPaymentSheet.show`), Modify `matching/presentation/widgets/create_bid_bottom_sheet.dart` (idem). `BidConfirmPaymentRequested` + navigation `?from=payment` inchangés (déclenchés par `onSuccess`).
