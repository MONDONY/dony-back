# DonyPaymentSheet — sheet de paiement custom — Design

## Contexte

L'app utilise la PaymentSheet préconstruite de Stripe (`initPaymentSheet` + `presentPaymentSheet`) sur 8 points d'appel. Le porteur produit veut **sa propre bottom sheet de paiement**, au design dony, tout en gardant Stripe comme processeur. Maquettes validées : `https://claude.ai/code/artifact/803c4d74-0439-4481-b873-8df685c1a349` (v2).
...
- **Règle bottom sheet du projet** : le bouton payer vit dans `stickyBottom` de `DonyBottomSheet`, jamais dans le `child` scrollable.
- 3DS : géré par `confirmPayment` (redirection auto), retour via le deep link existant `dony://stripe/payment-return`.

## Architecture

### Backend (dony-back) — deux ajouts, package `payments/`
...
### Flutter (dony_app) — feature `payments/`, composant réutilisable

**`DonyPaymentSheet.show(context, clientSecret, {amountLabel, contextLabel, onSuccess})`** — construit sur `DonyBottomSheet` :
...
- **Vue 2 (nouvelle carte)** : `CardFormField` natif stylé tokens dony ; toggle « Enregistrer cette carte » (le flag est transmis AVANT la création du PaymentIntent côté checkout — voir note flux) ; même stickyBottom.
